...
test("fillPath should add a key to an object two levels deep", () => {
    expect(fillPath({test: {one: {other: {}}}}, 'test.one.other', 'value')).toStrictEqual({test: {one: {other: 'value'}}});
});

test("fillPath should add a key to an object without prefixed keys", () => {
    expect(fillPath({}, 'test.one.other', 'value')).toStrictEqual({test: {one: {other: 'value'}}});
});
